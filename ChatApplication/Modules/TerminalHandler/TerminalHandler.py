...
class RoomTerminal(urwid.WidgetWrap):
    def __init__(self, interactDeployObj, chatroomName, username, public_address):
        ...
        self.screen_text = urwid.Text(
            self.chatRoomName + ' : ' + self.username)
    ...
    def getMsg(self):
        cnt = 0
        while self.flg:
            curr_msg = self.interactDeployObj.callGetMessagesFromChatRoomByName(
                self.interactDeployObj.chatRoomName, self.username, self.interactDeployObj.user_public_address)
            if curr_msg == None:
                ...
                for i in range(self.msgSize, len(curr_msg)):
                    if cnt > self.loop.screen.get_cols_rows()[1] - 7:
                        ...
                        try:
                            ...
                            print(
                                "Error to display messages, pleasse try again!" + str(e))
                    msg = curr_msg[i]
    ...
    def keypress(self, size, key):
        ...
        if key == 'enter':
            ...
            msg = query
            sendMsg = threading.Thread(
                target=self.interactDeployObj.transactAddMessage, args=(self.chatRoomName, msg))
            sendMsg.start()
            sendMsg.join()
            return
