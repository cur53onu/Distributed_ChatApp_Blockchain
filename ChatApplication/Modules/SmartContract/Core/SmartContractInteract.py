...
class SmartContractInteract:
    ...
    def transactDeployChatRoom(self, name):
        val = self.customTransact(self.getContractInstance().functions.deployChatRoom(name))
        if val[0]:
            self.checkStatusOfTransaction(val[1])
    def callGetChatRoomOwner(self, name):
        return self.getContractInstance().functions.getChatRoomOwner(name).call()
    ...
    def transactAddMessage(self,chatroom_name,username,message):
        ...
        val = self.customTransact(self.getContractInstance().functions.addMessage(chatroom_name,username,message,today))
        # if val[0]:
        #     self.checkStatusOfTransaction(val[1])
        return
    def callGetMessagesFromChatRoomByName(self,chatroom_name):
        return self.getContractInstance().functions.getMessagesFromChatRoomByName(chatroom_name).call()